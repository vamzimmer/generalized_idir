...
def create_tensor_from_torchio_subject(subject, image_names=('Fixed', 'Moving'), mask_names=('FixedMask', 'MovingMask')):
    images = []
    masks = []
    image_count = 0
    mask_count = 0
...
def data_transforms(**kwargs):
    ...
    if resampling:
        ...
        validate_tfm = tio.Compose([rescale])

    return intensity_tfm, validate_tfm


def intensity_transforms(**kwargs):

    # # default parameters and parameter parsing
    # whitening = kwargs["whitening"] if "whitening" in kwargs else True
    # rescaling = kwargs["rescaling"] if "rescaling" in kwargs else [-1, 1]
