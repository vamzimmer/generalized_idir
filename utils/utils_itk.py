...
def image_resample(image, downsample=0, transform=None, reference=None, out_origin=None, out_size=None,
                   out_spacing=None, interp='linear', default_intensity=0):
    """
    ...
    if reference is not None:
        resampled_img.SetDirection(reference.GetDirection())
...
class SmoothingPyramid(object):

    def __init__(self, schedule=None, sigma=0.7, interp='linear'):
        """
        Construction of a gaussian smoothing image pyramid.

        Arguments
        ---------
        `schedule` : downsampling schedule for pyramid
    ...
    def __call__(self, image):
        outputs = []
        assert isinstance(image, sitk.SimpleITK.Image), 'input not an image!'

        mini = np.amin(sitk.GetArrayFromImage(image))
        maxi = np.amax(sitk.GetArrayFromImage(image))

        for idx, fac in enumerate(self.schedule):
            if not fac==1:
                smoothed = self._smooth(image, self.sigma)
                outputs.append(self._resample(smoothed, fac, self.interp))
            else:
    ...
    def _smooth(self, img, sigma):
        ...
        gauss = gaussian.Execute(img)

        return gauss

    def _resample(self, img, sampling_factor, interp):
        if interp == 'linear':
            interp_func = sitk.sitkLinear
        elif interp == 'nearest':
            interp_func = sitk.sitkNearestNeighbor
        ...
        if img.GetDimension() == 3:
            out_spacing = [in_spacing[0] * sampling_factor,
                           in_spacing[1] * sampling_factor,
                           in_spacing[2] * sampling_factor]
            ...
            assert False, "wrong image dimension (" + str(img.GetDimension()) + "). Only dimensions 2 and 3 are supported."
